...
class OpenAICompatProvider:
    ...
    async def generate(self, request: GenerationRequest) -> GenerationResult:
        ...
        if not self._api_key:
            raise ProviderAuthError("OpenAI-compatible API key is not configured")
        ...
        try:
            ...
            raise ProviderTimeoutError(self._redact(f"OpenAI-compatible request timed out: {exc}")) from exc
        except httpx.RequestError as exc:
            raise ProviderUnavailableError(self._redact(f"OpenAI-compatible endpoint is unreachable: {exc}")) from exc
        latency_ms = (time.perf_counter() - started) * 1000

        status = response.status_code

        if status in (401, 402, 403, 404):
            raise ProviderAuthError(self._error_message(response, "OpenAI-compatible endpoint rejected the request"))

        if status == 200:
            try:
                data = response.json()
            except ValueError as exc:
                raise ProviderInvalidResponseError(self._redact(f"Unexpected response body: {exc}")) from exc

            if isinstance(data, dict) and "error" in data:
                code, message = _extract_error_info(data["error"])
                text = self._format_error("OpenAI-compatible endpoint returned an error body", status, code, message)
                if code == 429:
                    raise ProviderRateLimitError(text)
                raise ProviderUnavailableError(text)

            try:
                content = data["choices"][0]["message"]["content"]
            except (KeyError, IndexError, TypeError) as exc:
                raise ProviderInvalidResponseError(self._redact(f"Unexpected response shape: {exc}")) from exc
            if not content:
                raise ProviderInvalidResponseError("OpenAI-compatible response had empty content")
        ...
        if status == 429:
            raise ProviderRateLimitError(self._error_message(response, "OpenAI-compatible endpoint rate-limited"))
