...
| 18.2 Router roles/modes/budgets; Gemini removed | Coder | ✅ done (`16209c6`, `28196a4`) |
| 18.3 Settings + health + privacy note | Coder | ✅ done (`3e7c09f`, N1 `5301536`, N2 `4342bf3`) |
...
| 18.9 Vendor-aware requests + Gemini-first order (D30) | Coder | in progress (design approved) |
...
- 18.2 stop report (Coder, mid-implementation): four services' `AIMode.GEMINI` pre-flight guards would raise `AttributeError`, and, rewired, would contradict invariant 32. **Plan Amendment B:** delete the guards (plus their docstring line and unused imports, and tests asserting them) in `script_/learning_/thumbnail_/youtube_service.py`; keep the `settings.GEMINI_API_KEY` field until 18.3. The Coder correctly stopped rather than widening scope.
- 18.2 impl `16209c6` + N1 `28196a4` — **ACCEPTED** (PM, 2026-09-23). Amendments A and B are implemented. PM re-verification: full suite **1002 passed** on `16209c6`; `ruff` clean; real DB 7. **PM revert check found a test gap (N1):** a "fallback gets only the leftover" revert passed all 30 router tests, because the test used cloud 0.05 s < request 0.5 s, while production is cloud 150 s > request 120 s. N1 adds `test_fallback_gets_its_own_fresh_budget_not_the_leftover_of_the_primarys` (cloud 0.3 > request 0.2); the PM re-ran the same revert and it now FAILS (1 failed / 30 passed), restored → 31 passed. `record_success` is correctly moved to the primary. **Plan Amendment C:** `AI_ALLOW_CLOUD` defaults to true in 18.3; the `AI_MODE` default flips to `cloud_first` only at close-out after a Gate B-9 PASS.
- 18.3 design `d40e9c3` — **APPROVED with 3 changes** (PM, 2026-09-24). Accepted: 3 new `app_settings` rows with in-place overlay (they reach the next job via the per-dispatch builder); a write-only key with last4/source status; **old Gemini settings routes/functions/model deleted** (the `GEMINI_API_KEY` config field stays for the sample script); a 2-option mode selector (the cloud-only mode stays API/env-only); health reads the app-lifetime breaker; `tests/test_settings_browser.py`; an informational `check_cloud_provider()`. (C1) Test connection must **not regex-parse** the HTTP status out of 18.1's message text. Instead, 18.1's raised exceptions carry an `upstream_status` attribute (not `status_code`, which `AppError` uses for the HTTP response); `openai_compat_provider.py` and `tests/test_openai_compat_provider.py` are added to 18.3's allowed files for that attribute only. (C2) PUT validates the base URL with `validate_openai_compat_base_url` and the model as non-empty and bounded (≤ 200 chars); invalid input → 422 and nothing stored. (C3) The page shows the **effective** mode next to the selected one, with its reason (no key / kill switch off), so "cloud-first selected" never silently means local.
- 18.3 impl `3e7c09f` + N1 `5301536` + N2 `4342bf3` — **ACCEPTED** (PM, 2026-09-24). C1–C3 done; old Gemini settings routes/functions deleted (now 404/405). **N1 (found via the Coder's bug #1):** the pytest process loaded the owner's REAL OpenRouter key and URL from `.env` at import, and with `AI_ALLOW_CLOUD` now true, one unmocked Playwright route had reached the real API (one call, a 503). Fix: conftest neutralises the cloud config for the whole session (empty key, `openrouter.invalid` URL, `AI_ALLOW_CLOUD=False`, empty Gemini key), plus a guard test and a static scan. **N2:** the guard asserts key *lengths*, so a failure never prints the key. PM re-verification: full suite **1047 passed**, `ruff` clean, real DB 7. PM revert check (neutralisation removed, output captured to a file only) → guard FAILS showing "length 73; value hidden", 0 `sk-or` matches. `git log -p --all` contains no key.
- 18.4 design `f30b593` — **APPROVED with 2 changes** (PM, 2026-09-24). Job-level `fallback_used` is confirmed as maintained by `record_call` (ai_job_service.py:412), so reading it is valid; the runner's per-call recording already exists; the decision logic is unchanged (fallback rate not gated, D22). (C1) The window covers the last N **terminal** jobs, `complete` **and** `error` (not cancelled or stale), with per-status counts. Counting only completed jobs would hide exactly the jobs where the cloud failed *and* the fallback didn't save them. (C2) The runner test asserts that `DIE_AI_MODE` / `DIE_AI_ALLOW_CLOUD` are restored after the importlib fixture, so `--matrix cloud_first` can never leak `AI_ALLOW_CLOUD=true` into the pytest session.
- 18.4 impl `4e21554` — **ACCEPTED** (PM, 2026-09-24). C1 (terminal window + `by_status`) and C2 (env restore asserted) done. PM re-verification: PM revert check (window narrowed to `complete` → `test_get_fallback_rate_stats_includes_error_jobs_with_fallback_calls` FAILS, restored); full suite **1060 passed**; `ruff` clean. Next: **18.5 Gate B-9** (PM, Coder idle).
- 18.5 Gate B-9 — **EXECUTED** (PM, 2026-09-24), report `docs/operations/phase18-gate-b9.md`. The runner reports PASS (B1 5/5), but the plan criterion is **FAIL (regression)**: samples 3/4 (B-8: 4/4; sample B1 5-min died on malformed cloud JSON), and B1 wall time **280–828 s vs ~100–180 s local**. Accuracy improved while the cloud served (0–3 repairs vs 4–7). The **free quota (50 req/day, OpenRouter policy for accounts under $10 of credit) ran out mid-gate**; the remaining jobs completed on the local fallback, so invariant 32 is proven in the field. Fallback reasons: 15 rate-limit, 7 timeout (up to 150 s each), 1 unavailable. No key in any evidence file. **Per Amendment C the `AI_MODE` default stays `local`**; cloud_first is opt-in. Options A/B/C are in report §4 for the owner.
- 18.6 design `e72aec3` — **APPROVED with 4 changes** (PM, 2026-09-24). Accepted: the `models` array chain; a one-kwarg `adapter` on `_call_router` with a look-ahead parse and `router.generate_on_fallback()` (bypasses the circuit); `open_until()` on the same CircuitBreaker field; `ProviderDailyQuotaError` explicitly excluded from transient retry. (C1) Define `ProviderDailyQuotaError` in `app/core/exceptions.py` and map it to `provider_daily_quota` in `ai_job_service`'s provider-error mapping; both files are added to 18.6's allowed files for exactly that. (C2) "Cloud-served" is decided by the router (the primary provider's name or a router helper), not the literal `"ollama"`. (C3) Cap `open_until` at 26 h from now, against bad reset headers. (C4) `/api/ai/health` adds `circuit_open_until` (ISO, or null), and Settings shows "Cloud paused until HH:MM (free daily limit reached)"; `app/api/ai_jobs.py` (health only) is added.
- 18.6 impl `a17ca11` — **ACCEPTED** (PM, 2026-09-24). Chain via the `models` array, the answering model recorded, budget 75 s, daily-quota detection from the real shape, malformed-JSON local retry in `_call_router`, C1–C4 all done. PM re-verification: PM revert check (`open_until` → `record_failure` → both daily-quota circuit tests FAIL, restored); full suite **1112 passed**; `ruff` clean; real DB 7.
- 18.8 design `cc36c46` — **APPROVED** (PM, 2026-09-24), with rulings. Q1: a doc-built Gemini `RESOURCE_EXHAUSTED` fixture is accepted; the PM's pre-B-10 probe verifies it. Q2: `app/services/ai/contracts.py` is added (the `providers_tried` field only). Q3: `app/main.py` is added (the `_ai_circuits` dict only). Q4 semantics are defined by the PM: `circuit_open` = **all** configured cloud providers paused (the app is really running local); `circuit_open_until` = the **earliest** reopen time, only in that all-paused case, else null; per-provider detail goes under `providers`; the Settings "Cloud paused until" line appears only when all are paused, with per-provider notes otherwise. `OpenAICompatProvider.name` becoming an instance attribute is accepted. The worst case of 240 s (120 cloud + 120 local) is accepted. The per-provider fallback-rate breakdown in `ai_job_service` is **deferred** (the PM computes it per provider from B-10's per-call evidence). `GEMINI_MODEL` default `gemini-2.5-flash`.
- 18.8 impl `7f8676d` + N1 `889cddf` — **ACCEPTED** (PM, 2026-09-25). A multi-provider `ChainEntry` router with per-entry circuits, a total cloud budget of 120 s, the Amendment F defaults (openrouter, gemini ×2; Zen addable but off by default), Gemini array-wrapped error bodies handled, and the Q4 aggregate semantics. PM re-verification: PM revert check (the `min(…, remaining_total)` cap removed → `test_total_cloud_budget_caps_a_later_entrys_per_call_budget` FAILS, restored). **N1:** `ENV_GEMINI_API_KEY` was not neutralised in conftest; fixed; the PM revert check writes its output to a file only → guard FAILS with "length 53; value hidden", 0 secret-pattern matches. Full suite: first run 1158/1159, with one unidentified one-off failure that did not recur; second run **1159 passed**. `ruff` clean. The git history contains no key. The Gemini daily-quota detector is documented as **doc-based and unverified** until B-10's evidence.
- 18.7 Gate B-10 — **EXECUTED, FAIL** (PM, 2026-09-25), report `docs/operations/phase18-gate-b10.md`. The runner reads PASS, but under the Amendment D criteria: samples 3/4 (C1 repetition death on the local path) and a B1 median of 409 s vs ~150 s local (2.7×). **Root cause, verified:** the provider always sends the OpenRouter-only `reasoning` param, which Gemini rejects (400 INVALID_ARGUMENT "Unknown name reasoning"), so **Gemini served 0 of 118 calls**. That 400 was treated as a content error (retried), and the empty chain was mislabelled `no_cloud_provider_configured`. OpenRouter: 16 timeouts at 75 s. It was missed because the mock tests accept any field and the PM's probe used a hand-written payload (lesson: probe with the app's exact body). The default stays local. Proposed 18.9: a vendor-aware body, 400 → a non-transient request error, the label fix, and a chain-order decision for the owner.
